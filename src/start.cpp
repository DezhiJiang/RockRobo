...
namespace RockRobo{
    ...
    Start::Start(const RockRobo::StartOptions &start_options,
    ...
    void Start::AddSensorSamples(int trajectory_id,
                                 const RockRobo::TrajectoryOptions &options) {
    ...
    Start::ComputeExpectedSensorIds(const RockRobo::TrajectoryOptions &options,
    ...
    int Start::AddTrajectory(const RockRobo::TrajectoryOptions &options,
    ...
    bool Start::ValidateTrajectoryOptions(const RockRobo::TrajectoryOptions &options) {
...
//    bool Start::ValidateTopicNames(const std::map<std::string, std::string> topics,
//                                   const RockRobo::TrajectoryOptions &options) {
//        for(const auto& sensor_id : ComputeExpectedSensorIds(options, topics)){
//            const std::string& topic = sensor_id.id;
//            if(subscribed_topics_.count(topic) > 0){
//                LOG(ERROR) << "Toipc name [" << topic << "} is already used.";
//                return false;
//            }
//        }
//        return true;
//    }
    ...
    void Start::StartTrajectoryWithDefaultTopics(const RockRobo::TrajectoryOptions &options) {
        carto::common::MutexLocker lock(&mutex_);
...
//        }
    ...
    void Start::HandleOdometryMessage(int trajectory_id, const std::string &sensor_id,
        ...
        carto::common::MutexLocker lock(&mutex_);
        if(!sensor_samplers_.at(trajectory_id).odometry_sampler.Pulse()){
            Litelog(LEVEL_INFO, "Odom data Pulse, sensor_id:%s\n", sensor_id);
            return;
        }
        auto sensor_bridge_ptr = map_builder_bridge_.sensor_bridge(trajectory_id);
        sensor_bridge_ptr->HandleOdometryMessage(sensor_id, time, pData);
    }
    ...
    void Start::HandleLaserScanMessage(int trajectory_id, const std::string &sensor_id,
        ...
        map_builder_bridge_.sensor_bridge(trajectory_id)->
                HandleLaserScanMessage(sensor_id, time, pData);
    }

    void Start::HandleMultiEchoScanMessage(int trajectory_id, const std::string &sensor_id,
                                           double time, void *pData) {
        carto::common::MutexLocker lock(&mutex_);
        if(!sensor_samplers_.at(trajectory_id).rangefinder_sampler.Pulse()){
            return;
        ...
        map_builder_bridge_.sensor_bridge(trajectory_id)->
                HandleMultiEchoLaserScanMessage(sensor_id, time, pData);
    }
